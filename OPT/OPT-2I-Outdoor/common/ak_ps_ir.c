...
static void *ir_auto_switch_th(void *arg)
{
    ...
    // static bool prev_outdoor_online_state = false;
    // prev_outdoor_online_state = is_network_video_send_package_open();
	...
	while (ps_ctrl.thread_run) {
        ...
        if(cur_status == STATE_NIGHT && !is_network_video_send_package_open())
        {
            switch_day_or_night(STATE_DAY);
            // printf("=====================================>>>图像传输停止，切回白天 \n");
        }
        while(!is_network_video_send_package_open())
        {
            ak_sleep_ms(500);
        }
	}
