...
int GetSampleNum(unsigned char LayerNum, unsigned char VersionID)
{
	int i = 3 - LayerNum;
	int SampleNum = 0;
	if(VersionID == 3) SampleNum = SampleNumTable[i][0];
	if(VersionID == 2) SampleNum = SampleNumTable[i][1];
	if(VersionID == 0) SampleNum = SampleNumTable[i][2];
	return SampleNum;
}
...
{
	int SampleRate = 0;
	if(VersionID == 3) SampleRate = SampleRateTable[0][SampleRateIndex];
	if(VersionID == 2) SampleRate = SampleRateTable[1][SampleRateIndex];
	if(VersionID == 0) SampleRate = SampleRateTable[2][SampleRateIndex];
	return SampleRate;
}
...
int GetSideInfoSize(unsigned char ChannelMode, unsigned char VersionID)
{
	...
	if(VersionID == 3) 
	{
		if(ChannelMode == 3) SideInfoSize = SideInfoSizeTable[1][0];
		else SideInfoSize = SideInfoSizeTable[0][0];
	}
	else
	{
		if(ChannelMode == 3) SideInfoSize = SideInfoSizeTable[1][1];
		else SideInfoSize = SideInfoSizeTable[0][1];
	}
	return SideInfoSize;
}
...
bool music_info_get(char * path, mp3_info_t * mp3_info)
{
	...
	if(read(fd, ID3V2_head_buf, 10) < 0) wxj_log("file read error!\n");//获取ID3V2标签头
	...
	if(read(fd, ID3V1_buf, ID3V1_SIZE) < 0) wxj_log("file read error !\n");
	if((ID3V1_buf[0]=='T'||ID3V1_buf[0]=='t')
	&&(ID3V1_buf[1]=='A'||ID3V1_buf[1]=='a')
	&&(ID3V1_buf[2]=='G'||ID3V1_buf[2]=='g'))
	{
	...
	if(mp3_info->ID3V2_flag == true) start_pos = id3v2_info.ID3V2_size;
	else start_pos = 0;
	...
	case VOL_SUB_MODULE:
		if(user_data_get()->scene.bg_music_vol > 1)
		{
			...
			if(label != NULL)
			{
				...
				audio_volume_set((user_data_get()->scene.bg_music_vol));
			}
		}
		break;	
	case VOL_ADD_MODULE:
