...
{
    int count = 200;
    while (count-- && monitor_enter_way_get() == MONITOR_ENTER_TUYA)
    {
        if (!rtsp_stream_thread_flag && rtsp_stream_run_flag)
        {
            break;
        }
        usleep(1000 * 10);
...
static void live555_rtsp_stream_insert(unsigned char *data, int len)
{
    ...
    // if(rtsp_stream_open_succeed == false && video_decode_data_status())
    // {
    // printf("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA:%d = = %d\n",get_video_decode_state(),video_decode_data_status());
    //     rtsp_stream_open_succeed = true;
    // }

    if (/* get_video_decode_state() == true && rtsp_stream_open_succeed &&  */ tuya_event_state_get() != TRANS_LIVE_VIDEO_START)
    {
