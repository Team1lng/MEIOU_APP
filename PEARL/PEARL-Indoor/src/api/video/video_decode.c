...
static void video_decode_device_open(void)
{
	...
	int ret = ak_vdec_open(&param, &video_decode.handle_id);
	printf("Video Open ret :%d============.>>>\n", ret);
...
static void *video_decode_task(void *arg)
{
	...
	printf("%s============================.>>>%d\n", __func__, video_decode.handle_id);
	if (video_decode.handle_id <= 0)
	{
		printf("Video Open fail!!! Exit %s============.>>>\n", __func__);
...
bool video_decode_open(char type, int src_width, int src_height)
{
	...
	printf("[%s,%d] online_num = %d,state_get:%d\n", __func__, __LINE__, tuya_ipc_get_client_online_num(), tuya_event_state_get());
