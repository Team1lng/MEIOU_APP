...
static void draw_pattern(const lv_area_t * coords, const lv_area_t * clip, const lv_draw_rect_dsc_t * dsc)
{
    // printf("%s         dsc->pattern_image:%p           dsc->pattern_opa:%d\n",__func__,dsc->pattern_image,dsc->pattern_opa);
    ...
    int32_t img_h;
        ...
        // printf("%s     base_dir:%d    src_type:%d\n",__func__,dsc->base_dir,src_type);
    if(src_type == LV_IMG_SRC_FILE || src_type == LV_IMG_SRC_VARIABLE)
	{
        ...
        // label_dsc.bidi_dir = dsc->base_dir;
        lv_point_t s;
