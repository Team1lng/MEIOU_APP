...
extern void fb_gui_mode_enable(bool en);
extern void fb_bg_mode_enable(bool en);
...
#elif defined(MET_VERSION)
	lv_obj_set_style_local_text_font(time_obj_group[DATE_FOUMAT], LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, FONT_SIZE_L(36));
#endif
}
...
static void motion_back_btn_up(lv_obj_t *obj)
{
	...
	monitor_channel_set(MON_CH_NONE);
	fb_gui_mode_enable(false);
	fb_bg_mode_enable(false);
	goto_layout(pLAYOUT(standby));
}
...
static void LAYOUT_ENETER_FUNC(standby)
{
	...
	{
		standby_task_t = lv_task_create(standby_task, 120000, LV_TASK_PRIO_HIGH, NULL);
		lv_obj_invalidate(lv_scr_act());
...
static void LAYOUT_QUIT_FUNC(standby)
{
	...
	fb_gui_mode_enable(true);
	fb_bg_mode_enable(true);
}
