...
static void monitor_Gate2_btn_up(lv_obj_t *obj);
...
//新增GATE按钮
static char monitor_Gate2_task_lock_flag = 0;
static lv_task_t *ungate2_task_t = NULL;
static void monitor_Gate2_task(lv_task_t *task_t)
{
	printf("%s =================>>%d,%p,%d\n\r", __func__, __LINE__, ungate2_task_t, layout_is_monitor);
	if (!layout_is_monitor)
	{
		if (ungate2_task_t)
		{
		...
		monitor_Gate2_task_lock_flag = 0;
		return;
	}
	lv_obj_t *btn = lv_obj_get_child_form_id(lv_scr_act(), 9);
	static rom_bin_info info = rom_bin_info_get(ROM_RES_MONITOR_LOCK2_FOCUS_PNG);
	static rom_bin_info info1 = rom_bin_info_get(ROM_RES_MONITOR_LOCK2_UNFOCUS_PNG);
	lv_obj_set_style_local_pattern_image(btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, &info1);
	lv_obj_set_style_local_pattern_image(btn, LV_OBJ_PART_MAIN, LV_STATE_FOCUSED, &info);
	lv_obj_set_style_local_pattern_image(btn, LV_OBJ_PART_MAIN, LV_STATE_PRESSED, &info);
	lv_obj_clear_state(btn, LV_STATE_FOCUSED);
...
static void monitor_Gate2_btn_up(lv_obj_t *obj)
{
	...
	if (monitor_Gate2_task_lock_flag == 0)
	{
		// monitor_ring_timeout_val = 0;
		//audio_play_stop_set();
		lv_obj_t *btn = lv_obj_get_child_form_id(lv_scr_act(), 9);
		...
		unlock_gpio_set(1);
		tuya_dp_233_response_gate2(true);
		ungate2_task_t = lv_task_create(monitor_Gate2_task, user_data_get()->other.unlock_time * 1000, LV_TASK_PRIO_HIGH, NULL);
		monitor_Gate2_task_lock_flag = 1;
	}
}

// 创建GATE按钮
static void monitor_Gate2_btn_create(Controls_location coordinate)
{
	static btn_data btn_data = btn_data_create(NULL, monitor_Gate2_btn_up, NULL);
	static rom_bin_info info = rom_bin_info_get(ROM_RES_MONITOR_LOCK2_UNFOCUS_PNG);
	static rom_bin_info info1 = rom_bin_info_get(ROM_RES_MONITOR_LOCK2_FOCUS_PNG);
	lv_obj_t *btn = home_btn_create_1(coordinate, NULL, &btn_data, &info, &info1);
	lv_obj_set_id(btn, 9);
	monitor_Gate2_task_lock_flag = 0;
}
static void monitor_enter_ui_different_processing(void)
{
	...
	if (ch == MON_CH_DOOR_1 || ch == MON_CH_DOOR_2)
	{
		...
		{
			lv_obj_set_x(btn[0], 90);

			lv_obj_set_x(btn[1], 187);

			lv_obj_set_x(btn[2], 284);

			lv_obj_set_x(btn[3], 478);// lv_obj_set_hidden(btn[3], false);
			lv_obj_set_hidden(btn[3], false);

			lv_obj_set_x(btn[4], 575);// lv_obj_set_hidden(btn[4], false);
			lv_obj_set_hidden(btn[4], false);

            lv_obj_set_x(btn[5], 672);// lv_obj_set_hidden(btn[5], false);
			lv_obj_set_hidden(btn[5], false);

			lv_obj_set_x(btn[6], 769);

			lv_obj_set_x(btn[7], 866);

			lv_obj_set_x(btn[8], 381);
...
static void LAYOUT_ENETER_FUNC(monitor)
{
	...
	monitor_Gate2_btn_create(module_coordinate[GATE2_MODULE]);
