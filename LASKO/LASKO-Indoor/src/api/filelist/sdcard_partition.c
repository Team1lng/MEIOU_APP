...
bool _sdcard_format(int type)
{
	...
	if (type == 0)
	{
		if (access(SDMMC_DEVICE_PATH "/" SDMMC_DEVICE_NAME, F_OK) == 0)
		{
			Debug("mkdosfs -F 32  " SDMMC_DEVICE_PATH "/" SDMMC_DEVICE_NAME " -n LASKOMEX0 \n");
			system("mkdosfs -F 32  " SDMMC_DEVICE_PATH "/" SDMMC_DEVICE_NAME " -n LASKOMEX0 ");
		}
	}
	else if (type == 1)
	{
		...
		{
			Debug("mkdosfs -F 32  " SDMMC_DEVICE_PATH "/" SDMMC_PART_VDP_NAME " -n LASKOMEX1 \n");
			system("mkdosfs -F 32  " SDMMC_DEVICE_PATH "/" SDMMC_PART_VDP_NAME " -n LASKOMEX1 ");
		}
	}
	else
	{
		...
		{
			Debug("mkdosfs -F 32  " SDMMC_DEVICE_PATH "/" SDMMC_PART_TUYA_NAME " -n LASKOMEX2 \n");
			system("mkdosfs -F 32  " SDMMC_DEVICE_PATH "/" SDMMC_PART_TUYA_NAME " -n LASKOMEX2 ");
		}
	}
	system("sync");
	return true;
