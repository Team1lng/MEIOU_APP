...
bool GpioOpen(const int pin, GPIO_DIR dir, bool pull_enable)
{
    ...
    if (Direction[dir])
    {
        ...
        int len = sprintf(Value, "%s", Direction[dir]);
...
{
    const char DirStr[][9] = {"none\0", "rising\0", "falling\0", "both\0"};
    char Path[64];
    int Fd;
    ...
    if (Fd < 0)
    {
        printf("Pin:%d Failed to open gpio edge for writing!\n", pin);
        return -1;
    }

    if (write(Fd, &DirStr[edge], strlen(DirStr[edge])) < 0)
    {
        printf("Failed to set %s edge!!!\n", DirStr[edge]);
        return -1;
    }
