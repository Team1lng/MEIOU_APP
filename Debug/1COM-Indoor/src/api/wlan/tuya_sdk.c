...
static int tuya_dp_235_atmosphere(TY_OBJ_DP_S *dp)
{
    ...
    extern bool tuya_event_atmosphere(bool);
    tuya_event_atmosphere(dp->value.dp_bool);
    return 0;
}

int tuya_dp_235_response_atmosphere(BOOL_T state)
{
    printf("tuya_dp_235_response_atmosphere:%d\n\n", state);
    if (tuya_sdk_inited == false)
    {
        return -1;
    }
    BOOL_T param = state;
    return respone_dp_bool(BCOMTECH_DP_ATMOSPHERE, param);
}
...
static void tuya_dp_handle_func(const TY_RECV_OBJ_DP_S *dp_rev)
{
    ...
    case BCOMTECH_DP_LOCK_SUPPORT:
        ...
        break;
    case BCOMTECH_DP_ATMOSPHERE:
        printf("BCOMTECH_DP_ATMOSPHERE =================+++>\n");
        tuya_dp_235_atmosphere(dp_data);
        break;
...
static void tuya_status_change_func(const BYTE_T stat)
{
    ...
    case STAT_UNPROVISION:
        // tuya_dp_235_response_atmosphere(BOOL_T state);
        break;
