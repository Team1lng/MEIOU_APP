...
bool tuya_event_atmosphere(bool en)
{
	lv_event_info *node = lv_os_event_queue_node_new();
	if (node == NULL)
	{
	...
	node->msg.type = OS_EVENT_TYPE_TUYA;
	node->msg.arg1 = TUYA_EVENT_ATMOSPHERE;
	node->msg.arg2 = en;
	ak_thread_mutex_lock(&lv_os_event_queue_head_mutex);
	queue_insert((queue_s *)node, &lv_os_event_queue_head);
	ak_thread_mutex_unlock(&lv_os_event_queue_head_mutex);
...
{
	atmosphere_gpio_set(en);
	user_data_get()->atmosphere = en;
	user_data_save();
	extern int tuya_dp_235_response_atmosphere(BOOL_T state);
	tuya_dp_235_response_atmosphere(en);
}
