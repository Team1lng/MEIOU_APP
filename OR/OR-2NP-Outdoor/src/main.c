...
/*
autoor: zio
date：2025/12/23 设置新增五种欧洲语言的留言播报为英语
*/
...
void detect_message_status(void)
{
	...
	if (prev_status != message_status.open_message)
	{
		...
		if (prev_status)
		{
			/* 播放留言提示音 */
			int ring_index;
			if (message_status.message_language > 9 && message_status.message_language <= 14)
			{
				printf("message_language=%d, enter English ring branch\n", message_status.message_language);
				ring_index = RING_INDEX_MESSAGE_EH;
			}
			else
			{
				ring_index = RING_INDEX_MESSAGE_EH + message_status.message_language;
				printf("message_language=%d (<=9), ring_index=%d\n", message_status.message_language, ring_index);
			}
			play_doorbell(ring_index, 10);
			printf("play ring message start.......................%d\n\r", ring_index);
		}
		// 新增：当状态从“开启”→“关闭”时，强制重置标志位（解决未重置问题）
		else
		{
			message_status.open_message = prev_status = false;
			printf("message status reset to false\n");
		}
	}
	// 原“网络视频关闭时重置”的逻辑保留
...
int main(int argc, char *argv[])
{
	...
	/***** 初始化安凯平台中间件 *****/
	ak_platform_sdk_init();
	printf("[APP] 准备调用audio_output_device_init\n");
	/***** 初始化音频输出设备 *****/
	audio_output_device_init();
	printf("[APP] audio_output_device_init 调用成功\n");
	/***** 初始化音频采集设备 *****/
