 ...
 * @LastEditTime: 2024-04-28 07:46:19
...
bool Verify_upgrade_sha1sum(void)
{
...
bool Decompress_image_upgrade(void)
{
    char cmd[512] = {0};
...
bool start_upgrade_app(char *upgrade_file)
{
    ...
    {
        /* 2.校验升级压缩sha1值 */
        if (Verify_upgrade_sha1sum())
        {
            /* 3.解压image压缩包升级 */
            return Decompress_image_upgrade();
        }
    }
    return false;
}