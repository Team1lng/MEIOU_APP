...
#define TFTPD_CONF_NAME "tftpboot.conf"
#define TFTPD_CONF_PATH "/mnt/tf/tftpboot.conf"
...
static bool upgrade_thread_run = false;
...
void WriteTftpbootConf(const char *ip, const char *path)
{
    FILE *fp;

    // 打开文件以写入方式追加
    fp = fopen(TFTPD_CONF_PATH, "w");
    if (fp == NULL)
    {
    ...
    fprintf(fp, "bootfile=%s\n", path);

    // 关闭文件
    fclose(fp);

    printf("Data written to tftpboot.conf successfully.\n");
}

static int GetInterfaceIp(const char *if_name, char *ip_address)
{
    int sockfd;
    struct ifreq ifr;
    ...
    strncpy(ip_address, inet_ntoa(addr->sin_addr), INET_ADDRSTRLEN);

    return 0;
}
static void *CreateTftpdTask(void *arg)
{
    upgrade_info *info = (upgrade_info *)arg;

    char ip[16];
    if (GetInterfaceIp("br0", ip) != 0)
    {
        printf("获取本地IP地址失败\n");
        Upgrade_progress = UploadFail;
        upgrade_thread_run = false;
    ...
    WriteTftpbootConf(ip, info->path);
    // 定义tftp命令字符串
    char command[256];
    snprintf(command, sizeof(command), "tftp  -l %s -r %s -p %s 69", TFTPD_CONF_PATH, TFTPD_CONF_NAME, info->dest_ip);
    printf("%s\n", command);
    // 执行tftp命令
...
{
    if (upgrade_thread_run)
    {
        return false;
    }
    Upgrade_progress = NoneUpgrade;
    static upgrade_info info;
    memset(&info, 0, sizeof(info));
    memcpy(info.dest_ip, server_ip, strlen(server_ip));
    memcpy(info.path, path, strlen(path));
    upgrade_thread_run = true;
    int ret = pthread_create(&upgrade_client_thread_id, NULL, tftp ? CreateTftpdTask : CreateClientTask, (void *)&info);
