                               ...
                               IN PVOID_T pri_data)
{
    ...
    system("tar -zxvf /tmp/TWO_WIRE_APP -C /tmp;/tmp/update.sh &");
...
static void *tuya_wifi_sdk_init_task(void *arg)
{
    ...
    static int online_clinet_num = 0;
    static int monitor_timeout_upload = 0;
    ...
    tuya_ipc_set_region(REGION_CN);// 区域
    ...
    while (1)
    {
        ...
        //printf("online_clinet_num:%d \n", online_clinet_num);
        //printf("tuya_online_clinet_num_get():%d \n", tuya_online_clinet_num_get());
#ifndef _20241111_
        int num = tuya_ipc_get_client_online_num();
        //printf("%s tuya clinet onlin:%d user_clinet_online:%d,tuya_event_state_get():%d\n", __func__, tuya_ipc_get_client_online_num(), tuya_online_clinet_num_get(), tuya_event_state_get());
        if (num == 0 && tuya_online_clinet_num_get() != num)
#else
        if (online_clinet_num != tuya_online_clinet_num_get())
        {
            ak_get_ostime(&tv3);
            online_clinet_num = tuya_online_clinet_num_get();
            if(tuya_event_state_get() == TRANS_LIVE_VIDEO_START)
            {
                monitor_timeout_upload = 0;
                printf("Start Tuya monitor timer... \n");
            }
        }
        //printf("ak_diff_ms_time(&tv1, &tv3) is %ld\r\n",ak_diff_ms_time(&tv1, &tv3));
        if (tuya_event_state_get() == TRANS_LIVE_VIDEO_START && (ak_diff_ms_time(&tv1, &tv3) > 2 * 60 * 1000))
#endif
        {
            //printf("Tuya monitoring reset... \n");
            online_clinet_num = 0;
            ...
            monitor_timeout_upload = 3;
        }

        if (monitor_timeout_upload)
        {
            monitor_timeout_upload--;
            extern void tuya_timeout_upload(void);
            tuya_timeout_upload();
        }
