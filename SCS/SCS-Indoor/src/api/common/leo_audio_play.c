	...
	audio_info info =
		{
			...
			.volume = volume * VOLUME_INTERVAL + RING_VOLUME_MIN,
			.interrupt = true,
			.start = NULL,
			.end = NULL};
	audio_play(&info);
...
void chime_sound_play(int ring_index, int volume, audio_play_callback start, audio_play_callback end)
{
	...
	info.volume = volume * VOLUME_INTERVAL + RING_VOLUME_MIN; //?volume*5+30:0;
	info.start = start;
	info.end = end;
...
bool interphone_ring_play(int volume, bool send_net, audio_play_callback start, audio_play_callback end)
{
	...
	info.volume = volume * VOLUME_INTERVAL + RING_VOLUME_MIN;
	info.start = start;
	info.end = end;
	info.send_net = send_net;
	audio_play(&info);
...
{
	printf("%s===================>%d\n", __func__, vol);
	return audio_output_volume_set(vol * VOLUME_INTERVAL + (is_ring ? RING_VOLUME_MIN : VOLUME_MIN));
}
