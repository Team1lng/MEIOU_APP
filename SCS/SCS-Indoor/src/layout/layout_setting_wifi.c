...
WLAN_INFO_BTN WLAN_INFO_B = WLAN_BTN;
...
static void window_no_btn_up(lv_obj_t *obj)
{
	...
	lv_obj_t *window_cont = (lv_obj_t *)btn_data1->user_data;
	if (window_cont != NULL)
	{
...
static void tuya_unbind_yes_up(lv_obj_t *obj)
{
	...
	{
		extern OPERATE_RET http_gw_reset(IN CONST int stat);
		http_gw_reset(2);
...
static void tuya_unbind_msgbox_create(void (*window_yse_btn_up)(lv_obj_t *), char *str)
{
...
static void tuya_unbind_btn_down(lv_obj_t *obj)
{
	tuya_unbind_msgbox_create(tuya_unbind_yes_up, text_str(STR_UNBIND_HINT));
}

static void tuya_unbind_btn_create(void)
{
	...
	static btn_data btn_data1 = btn_data_create(tuya_unbind_btn_down, NULL, NULL);
	lv_obj_t *btn = lv_btn_create(lv_scr_act(), NULL);
	...
	lv_obj_set_style_local_value_str(btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, text_str(STR_UNBIND_HINT));
	lv_obj_set_style_local_value_align(btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, LV_ALIGN_CENTER);
	lv_obj_set_style_local_value_font(btn, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, FONT_SIZE(20));
...
static void close_wifi_connect(void)
{
	if (connect_falge)
	{
		...
		Debug("------------>>> new udhcpc  \n\n\n");
		system("killall udhcpc;udhcpc -i wlan0 -n 4 -R &");
		Debug("------------>>>  udhcpc  over\n\n\n");
	}
}
