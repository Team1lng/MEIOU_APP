...
static void monitor_2_head_create(void)
{
	...
	lv_cont_set_layout(cont, LV_LAYOUT_OFF);// 关闭自动布局
	lv_cont_set_fit(cont, LV_FIT_NONE);// 禁用自适应大小
	...
	{
		lv_task_del(monitor_timer_ptask);// 删除旧任务
	}
	monitor_timer_ptask = lv_task_create(monitor_timer_task, 500, LV_TASK_PRIO_HIGHEST, timer_label);// 创建新任务并设置高优先级
	lv_task_ready(monitor_timer_ptask);// 立即将任务加入就绪队列
	monitor_timer_task(monitor_timer_ptask);// 手动调用一次任务函数
}
...
static void monitor_switch_btn_up(lv_obj_t *obj)
{
	...
	else
	{
		...
		audio_talk_ctrl ctrl = {{ch + DEVICE_INDOOR_ID6}, (OPERATION_OPTION(AUDIO_SEND_EN) | OPERATION_OPTION(AUDIO_OUT_EN) | OPERATION_OPTION(AUDIO_RECEIVE_EN)), AI_C_AO_O, true, false, vol};
#else
...
static void window_volume_add_btn_up(lv_obj_t *obj)
{
	...
	else if (obj == lv_obj_get_child_form_id(window_cont, 54))
	{
		...
		sprintf(buf, "%02d", *vol);
		lv_label_set_text(label, buf);
		audio_ring_volume_set((*vol), monitor_ring_timeout_val ? true : false);
		// user_data_save();
	}
}
...
static void monitor_talk_btn_up(lv_obj_t *obj)
{
	...
	if (is_talking == false) // 未通话
	{
		if (montior_talk_task_t == NULL) // 如果通话任务未空
		{
...
#endif
			{
				...
				audio_play_stop_set();
#ifdef AUTO_ANSWER
				monitor_timer_set(MONITOR_DURATION - 5);
#else
	...
	else
	{
		...
		{
			network_device device = monitor_ch == MON_CH_DOOR_1 ? DEVICE_OUTDOOR_1 : DEVICE_OUTDOOR_2;
			audio_talk_ctrl ctrl = {{device}, (OPERATION_OPTION(AUDIO_SEND_EN) | OPERATION_OPTION(AUDIO_RECEIVE_EN) | OPERATION_OPTION(AUDIO_OUT_EN)), AI_C_AO_O, true, true, vol * VOLUME_INTERVAL + VOLUME_MIN};
			audio_talk_open(ctrl);
			audio_output_volume_set(vol * VOLUME_INTERVAL + VOLUME_MIN);
		}
	}
...
// system_bg_data_backup();
	...
	if (user_data_get()->other.model != AT_HOME_PATTERN)
	{
		audio_talk_ctrl ctrl = {{device}, (OPERATION_OPTION(AUDIO_SEND_EN) | OPERATION_OPTION(AUDIO_RECEIVE_EN) | OPERATION_OPTION(AUDIO_OUT_EN)), AI_C_AO_O, true, false, monitor_channel_get() == MON_CH_DOOR_1 ? user_data_get()->door1.talk_volume * VOLUME_INTERVAL + VOLUME_MIN : user_data_get()->door2.talk_volume * VOLUME_INTERVAL + VOLUME_MIN};
