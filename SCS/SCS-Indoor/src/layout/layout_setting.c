...
static void setting_1_atmosphere_light_btn_up(lv_obj_t *obj)
{
	static rom_bin_info info_off = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_OFF_PNG);
	static rom_bin_info info1_off = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_OFF_PRE_PNG);
	static rom_bin_info info_on = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_ON_PNG);
	static rom_bin_info info1_on = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_ON_PRE_PNG);

	if(atmosphere_state_read() == true)
	{
		printf("hlf:atmosphere off\n");
		atmosphere_gpio_set(false);
		lv_obj_set_style_local_pattern_image(obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, &info_off);
		lv_obj_set_style_local_pattern_image(obj, LV_OBJ_PART_MAIN, LV_STATE_PRESSED, &info1_off);
	}
	else
	{
		printf("hlf:atmosphere on\n");
		atmosphere_gpio_set(true);
		lv_obj_set_style_local_pattern_image(obj, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, &info_on);
		lv_obj_set_style_local_pattern_image(obj, LV_OBJ_PART_MAIN, LV_STATE_PRESSED, &info1_on);
	}
}

static void setting_1_atmosphere_light_btn_create(Controls_location **coordinate)
{
	static btn_data btn_data = btn_data_create(NULL, setting_1_atmosphere_light_btn_up, NULL);
	static rom_bin_info info = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_OFF_PNG);
	static rom_bin_info info1 = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_OFF_PRE_PNG);
	static rom_bin_info info_on = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_ON_PNG);
	static rom_bin_info info1_on = rom_bin_info_get(ROM_RES_SETTING_ATMOSPHERE_LIGHT_ON_PRE_PNG);
	if(atmosphere_state_read() == true)
		home_btn_create_1(**coordinate, text_str(STR_SYSTEM_AMBIENT_LED), &btn_data, &info_on, &info1_on);
	else
		home_btn_create_1(**coordinate, text_str(STR_SYSTEM_AMBIENT_LED), &btn_data, &info, &info1);
	(*coordinate)++;
}
...
static void LAYOUT_ENETER_FUNC(setting)
{
	...
	if(atmosphere_connect_get())
		setting_1_atmosphere_light_btn_create(&module_p);
