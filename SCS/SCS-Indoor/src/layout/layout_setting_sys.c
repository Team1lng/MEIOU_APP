...
static void mute_time_roller_btn_syn_prev(lv_obj_t *obj)
{
	btn_data *pdata = (btn_data *)obj->user_data;
	lv_obj_t *rooler = (lv_obj_t *)pdata->user_data;
	if (rooler == NULL)
...
static void mute_time_roller_btn_syn_next(lv_obj_t *obj)
{
	btn_data *pdata = (btn_data *)obj->user_data;
	lv_obj_t *rooler = (lv_obj_t *)pdata->user_data;
	if (rooler == NULL)
...
static lv_obj_t *mute_time_roller_create(lv_obj_t *parent, int x, int y, int w, char *opt, btn_data *btn_data2, btn_data *btn_data3)
{
	lv_obj_t *rooler = lv_roller_create(parent, NULL);
	lv_obj_set_style_local_text_line_space(rooler, LV_ROLLER_PART_BG, LV_STATE_DEFAULT, 20);
	lv_obj_set_style_local_text_color(rooler, LV_ROLLER_PART_BG, LV_STATE_DEFAULT, lv_color_make(0x80, 0x80, 0x80));
	...
	btn_data2->OPS_DOWN = mute_time_roller_btn_syn_prev;
	btn_data3->OPS_DOWN = mute_time_roller_btn_syn_next;
...
static void start_time_hour_roller_create(lv_obj_t *parent)
{
	...
	static btn_data btn_data2 = btn_data_create(NULL, NULL, NULL);
	static btn_data btn_data3 = btn_data_create(NULL, NULL, NULL);
	lv_obj_t *obj = mute_time_roller_create(parent, 72, 140, 80, opt, &btn_data2, &btn_data3);
	lv_obj_set_id(obj, 11);
	int id = user_data_get()->mute.timer_start / 100;
	lv_roller_set_selected(obj, id, LV_ANIM_OFF);
}
...
static void start_time_min_roller_create(lv_obj_t *parent)
{
	...
	static btn_data btn_data2 = btn_data_create(NULL, NULL, NULL);
	static btn_data btn_data3 = btn_data_create(NULL, NULL, NULL);
	lv_obj_t *obj = mute_time_roller_create(parent, 168, 140, 80, opt, &btn_data2, &btn_data3);
	lv_obj_set_id(obj, 22);
...
static void end_time_hour_roller_create(lv_obj_t *parent)
{
	...
	static btn_data btn_data2 = btn_data_create(NULL, NULL, NULL);
	static btn_data btn_data3 = btn_data_create(NULL, NULL, NULL);
	lv_obj_t *obj = mute_time_roller_create(parent, 417, 140, 80, opt, &btn_data2, &btn_data3);
	lv_obj_set_id(obj, 33);

	int id = user_data_get()->mute.timer_end / 100;
	lv_roller_set_selected(obj, id, LV_ANIM_OFF);
}
...
static void end_time_min_roller_create(lv_obj_t *parent)
{
	...
	static btn_data btn_data2 = btn_data_create(NULL, NULL, NULL);
	static btn_data btn_data3 = btn_data_create(NULL, NULL, NULL);
	lv_obj_t *obj = mute_time_roller_create(parent, 513, 140, 80, opt, &btn_data2, &btn_data3);
	lv_obj_set_id(obj, 44);
...
static void mute_time_set_ok_btn_up(lv_obj_t *obj)
{
	...
	user_data_get()->mute.timer_start = start_time_temp;
	user_data_get()->mute.timer_end = end_time_temp;
	static char str1[24] = {0};
	sprintf(str1, "%02d:%02d - %02d:%02d", user_data_get()->mute.timer_start / 100, user_data_get()->mute.timer_start % 100, user_data_get()->mute.timer_end / 100, user_data_get()->mute.timer_end % 100);

	lv_obj_t *btn = lv_obj_get_child_form_id(lv_scr_act(), 18);
	lv_obj_set_style_local_value_str(btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, str1);
}

static void mute_time_set_cancel_btn_up(lv_obj_t *obj)
{
	lv_obj_t *window_cont = lv_obj_get_child_form_id(lv_scr_act(), 888);
	if (window_cont != NULL)
	{
...
static void mute_time_setting_window_create(void)
{

	start_time_temp = user_data_get()->mute.timer_start;
	end_time_temp = user_data_get()->mute.timer_end;
	...
	static btn_data btn_data1 = btn_data_create(NULL, mute_time_set_ok_btn_up, NULL);
	...
	lv_obj_set_style_local_value_str(window_cancel_btn, LV_OBJ_PART_MAIN, LV_STATE_DEFAULT, text_str(STR_CANCEL));
	lv_obj_set_style_local_value_str(window_cancel_btn, LV_OBJ_PART_MAIN, LV_STATE_FOCUSED, text_str(STR_CANCEL));
	static btn_data btn_data2 = btn_data_create(NULL, mute_time_set_cancel_btn_up, NULL);
	window_cancel_btn->user_data = &btn_data2;
	btn_touch_event_listen(window_cancel_btn);
}

static void sys_mute_set_btn_up(lv_obj_t *obj)
{
	sys_set_btn_syn_up(obj);
	lv_obj_t *btn = lv_obj_get_child_form_id(lv_scr_act(), 18);
	char *str1 = {0};
	if (user_data_get()->mute.schedule)
	{
		...
		user_data_get()->mute.schedule = false;
	}
	...
	else if (user_data_get()->other.model == AT_HOME_PATTERN)
	{
		...
		str1 = buf;
		user_data_get()->mute.schedule = true;
		sprintf(str1, "%02d:%02d - %02d:%02d", user_data_get()->mute.timer_start / 100, user_data_get()->mute.timer_start % 100, user_data_get()->mute.timer_end / 100, user_data_get()->mute.timer_end % 100);
		time_t seconds = time(NULL);
		struct tm tm = {0};
		localtime_r(&seconds, &tm);
...
static void sys_mute_set_btn3_up(lv_obj_t *obj)
{
	if (user_data_get()->mute.schedule)
		mute_time_setting_window_create();
}
static void sys_mute_set_btn_create(Controls_location **coordinate)
{
	static btn_data btn_data1 = btn_data_create(sys_set_btn_syn_down, sys_mute_set_btn_up, NULL);
	static btn_data btn_data2 = btn_data_create(sys_set_btn_syn_down, sys_mute_set_btn_up, NULL);
	static btn_data btn_data3 = btn_data_create(sys_set_btn_syn_down, sys_mute_set_btn3_up, NULL);
	btn_data1.OPS_ANYTHING = sys_set_btn_syn_event;
	btn_data2.OPS_ANYTHING = sys_set_btn_syn_event;
	char *str1 = {0};
	...
	lv_obj_t *btn = sys_setting_btn_create(**coordinate, str1, text_str(STR_MUTE), &btn_data3, &btn_data1, &btn_data2);
	lv_obj_set_id(btn, 18);
	(*coordinate)++;
}
#else
...
#ifdef SCS_VERSION
	sys_mute_set_btn_create(&module_p);
#else
