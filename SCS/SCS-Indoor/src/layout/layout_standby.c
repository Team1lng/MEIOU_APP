...
#ifdef SCS_VERSION
static rom_bin_info info_intercom = rom_bin_info_get(ROM_RES_STANDY_TRANSFER_UNFOCUS_PNG);
static rom_bin_info info1_intercom = rom_bin_info_get(ROM_RES_STANDY_TRANSFER_FOCUS_PNG);

static rom_bin_info info_mute = rom_bin_info_get(ROM_RES_STANDY_MUTE_UNFOCUS_PNG);
static rom_bin_info info1_mute = rom_bin_info_get(ROM_RES_STANDY_MUTE_FOCUS_PNG);
static rom_bin_info info_mute_off = rom_bin_info_get(ROM_RES_STANDY_MUTE_OFF_UNFOCUS_PNG);
static rom_bin_info info1_mute_off = rom_bin_info_get(ROM_RES_STANDY_MUTE_OFF_FOCUS_PNG);
#endif
...
#ifdef SCS_VERSION
static rom_bin_info info_standby = rom_bin_info_get(ROM_RES_STANDY_STANDBY_UNFOCUS_PNG);
static rom_bin_info info1_standby = rom_bin_info_get(ROM_RES_STANDY_STANDBY_FOCUS_PNG);
#endif
...
#ifdef SCS_VERSION
static rom_bin_info *standby_model_info_get(int is_focus)
{
	if (user_data_get()->other.model == AT_HOME_PATTERN)
	{
		if (is_focus)
		{
			return &info_mute_off;
		}
		else
		{
			return &info1_mute_off;
		}
	}
...
static void standy_menu_btn_up(lv_obj_t *obj)
{
...
#ifdef SCS_VERSION
		audio_talk_ctrl ctrl = {{DEVICE_OUTDOOR_1}, (OPERATION_OPTION(AUDIO_SEND_EN) | OPERATION_OPTION(AUDIO_RECEIVE_EN) | OPERATION_OPTION(AUDIO_OUT_EN)), AI_C_AO_O, true, false, vol};
#else
	...
	case MUTE_MODULE:
	{
		...
		else if (user_data_get()->other.model == AT_HOME_PATTERN)
		{
			...
			user_data_get()->audio.ringback = false;
		}
		lv_imgbtn_set_src(obj, LV_BTN_STATE_RELEASED, standby_model_info_get(1));
		lv_imgbtn_set_src(obj, LV_BTN_STATE_PRESSED, standby_model_info_get(0));
	}
	break;
	case INTERCOM_MODULE:
...
#ifdef MEIOU_VERSION
#ifdef SCS_VERSION
	lv_obj_set_style_local_bg_opa(cont, LV_LABEL_PART_MAIN, LV_STATE_DEFAULT, LV_OPA_0);
#else
...
#ifdef SCS_VERSION
static lv_obj_t *MUTE_MODULE_CREATE(user_obj *obj)
{
	return standy_shortcut_btn_create(obj, FILE_TYPE_NONE, standby_model_info_get(1), standby_model_info_get(0));
}

static lv_obj_t *INTERCOM_MODULE_CREATE(user_obj *obj)
{
	return standy_shortcut_btn_create(obj, FILE_TYPE_NONE, &info_intercom, &info1_intercom);
}
#else
...
#ifdef SCS_VERSION
static lv_obj_t *STANDBY_MODULE_CREATE(user_obj *obj)
{
	return standy_shortcut_btn_create(obj, FILE_TYPE_NONE, &info_standby, &info1_standby);
}
...
#ifdef SCS_VERSION
	[MUTE_MODULE] = {.area = {0, 15, SHORTCUT_KEY_WIDTH, SHORTCUT_KEY_WIDTH}, .parent = &USER_OBJ_GET(STORTCAT_OBJ, SHORTCUT_CONT), USER_OBJ_CONSTRUCTOR(MUTE_MODULE)},
	[INTERCOM_MODULE] = {.area = {0, 15, SHORTCUT_KEY_WIDTH, SHORTCUT_KEY_WIDTH}, .parent = &USER_OBJ_GET(STORTCAT_OBJ, SHORTCUT_CONT), USER_OBJ_CONSTRUCTOR(INTERCOM_MODULE)},
#else
...
#endif
#ifdef SCS_VERSION
	[STANDBY_MODULE] = {.area = {0, 15, SHORTCUT_KEY_WIDTH, SHORTCUT_KEY_WIDTH}, .parent = &USER_OBJ_GET(STORTCAT_OBJ, SHORTCUT_CONT), USER_OBJ_CONSTRUCTOR(STANDBY_MODULE)},
#endif
};
